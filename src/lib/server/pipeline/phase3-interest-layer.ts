...
const BUCKET_DESCRIPTIONS: Record<PriorityBucket, string> = {
  P0: '高信任创作者（优先考虑，但不强制进入必读）。',
  P1: '多源热点簇（多个来源同时报道的主题，是必读的强信号）。',
  P2: '单一来源的系列或专题（放在"仅供参考"或作为补充信息）。',
  P3: '其他内容（可能包含有价值的独立文章，不要完全忽略）。',
};

const EDITORIAL_STYLE_GUIDANCE = [
  '必读应优先选择多源共鸣（P1）和时效性强的内容。',
  'P0 创作者的更新值得关注，但必读不应只看来源，要看内容本身的价值。',
  '从 P2/P3 中也可能发现有深度的独立文章，不要忽略。',
].join(' ');
