...
<div class="space-y-6">
  <!-- OPML Upload -->
  <div>
    <h3 class="text-lg font-medium text-gray-900 mb-3">上传OPML文件</h3>
    <div
      class="border-2 border-dashed rounded-lg p-8 text-center transition-colors {dragOver ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'}"
      ondrop={handleDrop}
    ...
    >
      ...
      {:else}
        ...
        <p class="mt-2 text-sm text-gray-500">支持 OPML / XML 格式</p>
      {/if}
    </div>
  </div>
  ...
  <!-- Add Single URL -->
  <div>
    <h3 class="text-lg font-medium text-gray-900 mb-3">添加单个RSS链接</h3>
